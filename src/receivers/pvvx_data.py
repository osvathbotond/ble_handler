...
try:
    ...
    class PVVXData(Receiver):
        ...
        def check_type(message: bytes) -> bool:
            ...
            try:
                return (
                    message_bytearray[0] == 18
                    and int.from_bytes(
                        message_bytearray[2:4], byteorder="little", signed=False
                    )
                    == 6170
                )
            except IndexError:
        ...
        def from_message(cls, message: bytes):
            ...
            return cls(
                ...
                temperature=int.from_bytes(
                    message_bytearray[10:12], byteorder="little", signed=True
                ),
                humidity=int.from_bytes(
                    message_bytearray[12:14], byteorder="little", signed=False
                ),
                battery_mv=int.from_bytes(
                    message_bytearray[14:16], byteorder="little", signed=False
                ),
